...
def login():
    if request.method == "POST":
        ...
        cursor.execute('SELECT * FROM users WHERE username = ? AND pass = ?', (username, passwd))
...
def getRecentImages():
    ...
    cursor.execute('SELECT * FROM images WHERE setting = ? ORDER BY imageID DESC', ("public",))
...
def downloadImage(imageID):
    ...
    cursor.execute('SELECT imageName FROM images WHERE imageID = ?', (imageID,))
...
def profileimages(user):
    ...
    if session['username'] == user:
        ...
        cursor.execute('SELECT * FROM images WHERE username = ?', (user,))
        userimages = cursor.fetchall()
        # print(userimages)
    
    else:
        # Grab from new table here
        cursor.execute('SELECT * FROM images WHERE username = ? AND setting = ?', (user, "public"))
        userimages = cursor.fetchall()
        # print(userimages)
    ...
    for images in userimages:
        imageID = images[1]
        # print(imageID)
        cursor.execute('SELECT * FROM meta WHERE imageID = ?',(imageID,))
    ...
    for images in userimages:
        ...
        cursor.execute('SELECT * FROM meta WHERE imageID = ?',(images[1],))
        metadata = cursor.fetchall()
...
def settings():
    ...
    if request.method == "POST":
        ...
        cursor.execute('SELECT pass FROM users WHERE username = ?', (session['username'],))
        ...
        if newUser1 and newUser2 and newPass1 and newPass2:
            if not newUser1 == newUser2:
                ...
                return redirect('/settings')
            cursor.execute('UPDATE users SET username = ?, pass = ? WHERE username = ?', (newUser1, newPass1, session['username']))
            cursor.execute('UPDATE images SET username = ? WHERE username = ?', (newUser1, session['username']))
            conn.commit()
        ...
        if newUser1 and newUser2:
            ...
            cursor.execute('SELECT * FROM users WHERE username = ?', (newUser1,))
            all_users = cursor.fetchall()
            if len(all_users) > 0:
                return redirect('/settings')

            cursor.execute('UPDATE users SET username = ? WHERE username = ?', (newUser1, session['username']))
            cursor.execute('UPDATE images SET username = ? WHERE username = ?', (newUser1, session['username']))
            conn.commit()
            session['username'] = newUser1
            return redirect('/settings/updated')
        ...
        if newPass1 and newPass2:
            if not newPass1 == newPass2:
                return redirect('/settings')
            cursor.execute('UPDATE users SET pass = ? WHERE username = ?', (newPass1, session['username']))
            conn.commit()
            return redirect('/settings/updated')
...
def mosaicify(privacy, user_image):
    ...
    cursor.execute('SELECT * FROM images WHERE imageName = ?', (new_img,))
    poss_images = cursor.fetchall()

    image_num = ""

    if len(poss_images) > 0:
        cursor.execute('SELECT MAX(num) FROM images WHERE imageName = ?', (new_img,))
...
def results(privacy, user_image, image_num):
    ...
    if request.method == "POST":
        ...
        if image_num == "":
            cursor.execute('INSERT INTO images (username, imageID, setting, imageName, num, date) VALUES (?, ?, ?, ?, ?, ?)', (session['username'], newID, privacy, user_image, 0, curr_date))
        else:
            cursor.execute('INSERT INTO images (username, imageID, setting, imageName, num, date) VALUES (?, ?, ?, ?, ?, ?)', (session['username'], newID, privacy, user_image, image_num, curr_date))
...
def view(image_id):
    ...
    cursor.execute('SELECT * FROM images WHERE imageID = ?', (image_id,))
    image_info = cursor.fetchall()
    ...
    temp_date = image_info[0][5]
    new_date = datetime.strptime(temp_date, "%Y-%m-%d")
    curr_date = new_date.strftime("%m/%d/%Y")
